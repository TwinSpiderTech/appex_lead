...
class GenericFormController extends GetxController {
  ...
  var manuallyEditedFields = <String>{}.obs;
  ...
  void resumeDraft(dynamic draftData) {
    ...
    // When resuming, treat existing non-null values as manually edited to prevent overwriting
  ...
  void updateFieldValue(
    ...
    debugPrint(
      "UPDATING FIELD: $fieldName -> $value (Automatic: $isAutomatic)",
    );

    if (!isAutomatic) {
      manuallyEditedFields.add(fieldName);
      debugPrint("MARKED AS MANUALLY EDITED: $fieldName");
    }
